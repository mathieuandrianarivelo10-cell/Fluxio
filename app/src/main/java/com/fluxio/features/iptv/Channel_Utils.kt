...
fun sortFrenchChannelsFirst(channels: List<LiveChannel>): List<LiveChannel> {
    val tntPriority = mapOf(
        "tf1" to 1,
        "france 2" to 2,
        "france 3" to 3,
        "canal+" to 4, "canal plus" to 4,
        "france 5" to 5,
        "m6" to 6,
        "arte" to 7,
        "c8" to 8,
        ...
        "tmc" to 10,
        "tfx" to 11,
        "nrj 12" to 12, "nrj12" to 12,
        "lcp" to 13, "public senat" to 13,
        "france 4" to 14,
        "bfm tv" to 15, "bfmtv" to 15, "bfm" to 15,
        "cnews" to 16,
        "cstar" to 17,
        "gulli" to 18,
        "tf1 series" to 20, "tf1 séries" to 20,
        "l'equipe" to 21, "l'équipe" to 21, "equipe" to 21,
        "6ter" to 22,
        "rmc story" to 23,
        "rmc decouverte" to 24, "rmc découverte" to 24,
        "cherie 25" to 25, "chérie 25" to 25,
        "lci" to 26,
        "france info" to 27, "franceinfo" to 27
    )

    fun getTntRank(name: String): Int? {
        val lower = name.lowercase().trim()
        for ((key, rank) in tntPriority) {
            if (lower == key || lower.startsWith("$key ") || lower.contains(" $key ") || lower.contains(key)) {
                return rank
            }
        }
        return null
    ...
    fun isFrenchChannel(channel: LiveChannel): Boolean {
        val country = channel.country.lowercase().trim()
        val name = channel.name.lowercase().trim()
        val category = channel.category.lowercase().trim()
        return country == "fr" || country == "france" ||
                name.contains("fr") || name.startsWith("fr:") || name.startsWith("fr -") ||
                category.contains("fr") || category.contains("france") ||
                getTntRank(name) != null
    }

    val tntChannels = mutableListOf<Pair<Int, LiveChannel>>()
    val otherFrenchChannels = mutableListOf<LiveChannel>()
    val nonFrenchChannels = mutableListOf<LiveChannel>()

    for (channel in channels) {
        val tntRank = getTntRank(channel.name)
        if (tntRank != null) {
            tntChannels.add(tntRank to channel)
        } else if (isFrenchChannel(channel)) {
            otherFrenchChannels.add(channel)
        } else {
            nonFrenchChannels.add(channel)
        }
    }

    tntChannels.sortBy { it.first }
    otherFrenchChannels.sortBy { it.name.lowercase() }
    nonFrenchChannels.sortBy { it.name.lowercase() }

    val result = mutableListOf<LiveChannel>()
    result.addAll(tntChannels.map { it.second })
    result.addAll(otherFrenchChannels)
    result.addAll(nonFrenchChannels)

    return result.mapIndexed { index, channel ->
        channel.copy(channelNumber = index + 1)
    }
}
